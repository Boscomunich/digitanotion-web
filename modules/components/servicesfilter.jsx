...
export default function ServicesFilter({ activeKey, onChange, className = "" }) {
  return (
    <div className={`w-full flex flex-wrap items-center justify-center gap-2 sm:gap-3 ${className}`}>
      {SERVICES.map((svc) => {
        ...
        return (
          <button
            ...
            style={{ "--active": ACTIVE_COLOR, backgroundColor: active ? ACTIVE_COLOR : undefined }}
          >
            {svc.name}
          </button>
        );
